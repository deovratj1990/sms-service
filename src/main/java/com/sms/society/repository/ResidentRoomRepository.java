...
public interface ResidentRoomRepository extends JpaRepository<ResidentRoom, Long> {
	@Query("select rr from ResidentRoom rr inner join rr.resident res inner join res.user u where rr.status = com.sms.society.entity.ResidentRoom$Status.ACTIVE and u.id = :userId")
    public List<ResidentRoom> findByUserId(@Param("userId") Long userId);
